...
from the original CRM workbook ("需要的示例产品" column) and are matched with
...
DEFAULT_CATALOG_PATH = Path(
    os.getenv(
        ...
        str(Path(__file__).resolve().parents[2] / "config" / "product_catalog.yaml"),
    )
)
...
def _safe_catalog_filename(category: ProductCategory, suffix: str) -> str:
    ...
    return f"Example_Chemicals_{category_token}_product_list.{suffix}"
