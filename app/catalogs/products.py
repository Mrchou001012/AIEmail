...
DEFAULT_PRODUCT_ALIASES_PATH = Path(
    os.getenv(
        "PRODUCT_ALIASES_PATH",
        str(Path(__file__).resolve().parents[2] / "config" / "product_aliases.yaml"),
    )
)
...
def find_product_codes(text: str, path: str = str(DEFAULT_PRODUCT_ALIASES_PATH)) -> list[str]:
    ...
    # "GRADE-A(99%)" from also being interpreted as the shorter GRADE-A -> 98% alias.
