...
def test_private_email_case_quote_coa_and_packing_are_all_preserved() -> None:
    result = stub_analyze(
        "Re: Checking in",
        "Please quote for 3 MT of DEMO-P302 with current COA and packing details.",
        [],
    )
    ...
    assert result.product_code == "DEMO-P302"
...
def test_private_email_case_multi_quote_keeps_per_product_quantities_and_coa() -> None:
    ...
    assert [line.model_dump() for line in result.product_requests] == [
        {"product_code": "DEMO-P301", "quantity": 2000},
        {"product_code": "DEMO-P302", "quantity": 3000},
    ]
...
def test_quantity_only_revision_is_not_a_counteroffer() -> None:
    ...
    normalized = _normalize_quantity_revision(
        misclassified,
        "Please quote 800 kg DEMO-P200 instead.",
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Please quote DEMO-P100 and include the CAS number.", False),
        ...
        (
            ...
            "On Mon, 31 Aug 2026, sales@example.com wrote:\n"
...
def test_stub_draft_preview_is_review_only_and_ignores_historical_prices() -> None:
    ...
    assert "600 kg of DEMO-P200" in rendered
...
def test_product_list_draft_uses_approved_category_and_attachment() -> None:
        ...
        ai.draft_preview(
            {
                ...
                "approved_product_catalog": [
                    {"code": "DEMO-SO", "name": "Silicone Oil"}
                ],
                "approved_attachments": [
                    {
                        "filename": "Example_Chemicals_silicone_oil_product_list.xlsx",
    ...
    assert "Example_Chemicals_silicone_oil_product_list.xlsx" in rendered
...
def test_demo_end_to_end_flow() -> None:
    ...
    assert "Demo Sales Team" in parsed.body_text
    assert "This is a fictional signature for local development" in parsed.body_text
