...
async def test_coa_human_correction_resumes_to_verified_review_draft(
    ...
    product_dir = root / "SILANES" / "DEMO-TEST"
    product_dir.mkdir(parents=True)
    (product_dir / "COA-DEMO-TEST customer.pdf").write_bytes(b"customer-specific")
    catalog_path = tmp_path / "runtime" / "coa_catalog.json"
    monkeypatch.setattr(
        "app.coa.coa_catalog.extract_document_bounded",
        lambda path, timeout_seconds: "Product DEMO-TEST",
    )
    ...
    scanner.scan()
    lookup = COACatalog(catalog_path).find("DEMO-TEST")
    ...
    email = EmailMessage(
        ...
        subject="COA for DEMO-TEST",
        body_text="Please send the COA for DEMO-TEST.",
        attachment_metadata=[],
        raw_sha256="1" * 64,
    )
    ...
    analysis = InboundAnalysis(
        ...
        coa_requested=True,
        requested_product_name="DEMO-TEST",
    ...
    (product_dir / "COA-DEMO-TEST.pdf").write_bytes(b"approved-standard")
    scanner.scan()
...
async def test_mixed_request_can_mark_missing_coa_for_manual_reply(
    ...
    email = EmailMessage(
        ...
        body_text="Please quote and send the COAs for DEMO-P302 and DEMO-P301.",
        attachment_metadata=[],
        raw_sha256="2" * 64,
    )
    db_session.add(email)
    ...
    handoff = await create_handoff(
        ...
        facts={
            **analysis.model_dump(mode="json"),
            "missing_coa_queries": ["DEMO-P301"],
            "prepared_coas": [{"product_name": "DEMO-P302"}],
