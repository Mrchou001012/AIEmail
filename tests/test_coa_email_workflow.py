...
def _analysis(product_name: str | None = "DEMO-P900") -> InboundAnalysis:
...
async def test_coa_request_creates_review_only_draft_with_pinned_attachment(monkeypatch, tmp_path):
    entry = {
        "path": "PAPER CHEMICALS/DEMO-P900/COA-DEMO-P900.pdf",
        "product_name": "DEMO-P900",
        "product_code": "DEMO-P900",
        "sha256": "a" * 64,
        "size": 1234,
    }
    ...
    response = PreparedCOAResponse(
        subject="COA for DEMO-P900",
        body_text="Please find attached the requested COA.",
        product_names=("DEMO-P900",),
        prepared_coas=(prepared,),
        attachments=(),
        lookups=({"query": "DEMO-P900", "result": result.as_dict()},),
        catalog_schema="coa-catalog.v1",
    )
    ...
    email = SimpleNamespace(id=8, subject="COA for DEMO-P900")
...
async def test_coa_request_with_no_unique_match_asks_for_specific_human_help(monkeypatch, tmp_path):
    ...
    await services._maybe_handle_coa_request(
        ...
        case=case,
        email_row=email,
        analysis=_analysis("DEMO-P100"),
        analysis_facts=_analysis("DEMO-P100").model_dump(mode="json"),
    )
...
async def test_secondary_coa_request_queues_verified_reply_and_allows_quote_to_continue(
    ...
    response = PreparedCOAResponse(
        subject="Re: DEMO-P302 quotation",
    ...
    analysis = _analysis("DEMO-P302").model_copy(
    ...
    email = SimpleNamespace(
        id=18,
        subject="DEMO-P302 quotation",
        body_text="Please quote DEMO-P302 and send the COA.",
...
async def test_partial_coa_reply_sends_available_file_and_creates_missing_item_handoff(
    ...
    response = PreparedCOAResponse(
        ...
        body_text=(
            ...
            "The COA for DEMO-P301 is not included in this email."
        ),
        product_names=("DEMO-P302",),
        prepared_coas=(prepared,),
        attachments=(attachment,),
    ...
    email = SimpleNamespace(
        id=19,
        subject="COAs",
        body_text="Please quote and send DEMO-P302 and DEMO-P301 COAs.",
        from_address="buyer@example.com",
        received_at=datetime.now(UTC),
    ...
    assert handoff_call["facts"]["missing_coa_queries"] == ["DEMO-P301"]
