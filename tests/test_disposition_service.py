...
async def test_apply_departed_suppresses_old_endpoint_and_saves_referral(
    ...
    _, contact = await _seed_contact(
        ...
        name="Former Contact",
        email="former@film-maker.example",
    )
    row = _email(
        contact=contact,
        subject="Automatic reply: Checking in",
        body=(
            "Former Contact is no longer employed here. Please direct future correspondence "
            "to Replacement Contact at replacement@film-maker.example. This email has been automatically "
            "forwarded to Replacement Contact."
        ),
        token="c",
        auto=True,
    )
    ...
    assert referral.referred_email == "replacement@film-maker.example"
...
async def test_apply_out_of_office_sets_resume_boundary(
    ...
    _, contact = await _seed_contact(
        ...
        name="Barbara",
        email="barbara@contact-change.example",
    )
    row = _email(
        contact=contact,
        subject="Automatic reply: Checking in",
...
async def test_cross_domain_forward_resolves_exact_reactivation_parent_for_review(
    ...
    customer, original = await _seed_contact(
        db_session,
        company="Legacy Buyer (Now Example Buyer)",
        name="Original Buyer",
        email="purchase@adhesives-customer.example",
    )
    ...
    parent = Outbox(
        ...
        message_id="<cross-domain-parent@demo-company.example>",
    ...
    row = EmailMessage(
        direction="INBOUND",
        message_id="<cross-domain-reply@unrelated-customer.example>",
        in_reply_to=parent.message_id,
        references_json=[parent.message_id],
        from_address="former@adhesives.example",
        to_addresses=["sales@example.com", "replacement@adhesives.example"],
        subject="RE: Checking in",
        body_text=(
            "I have marked copy to Morgan in this communication. "
    ...
    disposition = await classify_email_disposition(
        ...
        ai_client=_DispositionAI(
            InboundDispositionDecision(
                ...
                evidence=["I have marked copy to Morgan"],
                replacement_emails=[],
                return_hint=None,
                forwarded_to_replacement=False,
                non_target_reason=None,
    ...
    assert plan["customer_name"] == "Legacy Buyer (Now Example Buyer)"
    assert plan["contact_id"] == original.id
    assert plan["contact_email"] == "purchase@adhesives-customer.example"
    ...
    assert plan["replacement_emails"] == ["replacement@adhesives.example"]
    ...
    assert referral.referred_email == "replacement@adhesives.example"
    assert referral.forwarded_already is True
    assert referral.status == "WAITING_FOR_FORWARDED_REPLY"
    assert await db_session.scalar(
        select(func.count())
...
async def test_quoted_parent_recovers_departed_contact_and_continues_product_list(
    ...
    customer, original = await _seed_contact(
        db_session,
        company="Example Buyer Co., Ltd.",
        name="Pooja Raut",
        email="original@buyer-company.example",
    )
    outbound_body = (
        "Dear Pooja Raut,\n\n"
    ...
    outbound = EmailMessage(
        ...
        contact_id=original.id,
        direction="OUTBOUND",
        message_id="<quoted-parent@example.com>",
        from_address="demo.agent@example.org",
    ...
    inbound = EmailMessage(
        direction="INBOUND",
        message_id="<reply-without-thread-headers@buyer-company.example>",
        from_address="reply@buyer-company.example",
        to_addresses=["demo.agent@example.org"],
        subject="Checking in from Example Chemicals",
        body_text=(
            "Dear Demo Agent,\n\n"
            ...
            "Example Buyer Co., Ltd.\n\n"
            "From: demo.agent@example.org\n"
            "Date: August 3, 2026\n"
            "To: original@buyer-company.example\n"
            "Subject: Checking in from Example Chemicals\n\n"
            f"{outbound_body}"
        ),
        attachment_metadata=[],
    ...
    assert plan["reply_contact_candidate_email"] == "reply@buyer-company.example"
    assert plan["reply_contact_candidate_name"] == "Taylor"
    ...
    reply_contact = await db_session.scalar(
        select(Contact).where(Contact.email == "reply@buyer-company.example")
    )
    ...
    assert reply_contact.name == "Taylor"
    ...
    assert await db_session.scalar(
        select(func.count())
        .select_from(Contact)
        .where(Contact.email == "reply@buyer-company.example")
    ) == 0
    assert await db_session.scalar(
        select(func.count())
        .select_from(ContactReferral)
...
async def test_copied_same_domain_recipient_is_audited_without_duplicate_outreach(
    ...
    _, contact = await _seed_contact(
        ...
        name="Former Contact",
        email="former@adhesives.example",
    )
    row = _email(
        contact=contact,
        subject="RE: Checking in from Example Chemicals",
        body=(
            "Mr Girish Dalal has already retired and Mr Replacement Contact is now "
            "your contact point. I have marked copy to Morgan in this "
    ...
    disposition = await classify_email_disposition(
        ...
        ai_client=_DispositionAI(
            InboundDispositionDecision(
                disposition_type="CONTACT_REFERRAL",
                confidence=0.96,
                reason="The sender identifies Morgan as the new contact.",
                evidence=["I have marked copy to Morgan in this communication"],
                replacement_emails=[],
                return_hint=None,
                forwarded_to_replacement=False,
                non_target_reason=None,
    ...
    assert disposition.replacement_emails == ("replacement@adhesives.example",)
    assert (
        "REPLACEMENT_FROM_RECIPIENT_HEADER:replacement@adhesives.example"
...
async def test_multiple_referral_addresses_have_canonical_order(
    ...
    _, contact = await _seed_contact(
        ...
        name="Former Buyer",
        email="former@distributor.example",
    )
    row = _email(
        contact=contact,
        ...
        body=(
            ...
            "Please send your order to orders@order-desk.example. If you need assistance, "
            "please send your email to help-one@distributor.example or "
