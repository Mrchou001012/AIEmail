...
def test_mime_strips_wecom_localized_quoted_history() -> None:
    ...
    message.set_content(
        ...
        "demo.agent<demo.agent@example.org&gt;&nbsp;"
    ...
    assert parsed.body_text == "Please quote 800 kg DEMO-P200 instead."
...
def test_build_message_embeds_signature_logo() -> None:
    ...
    message = BytesParser(policy=policy.default).parsebytes(raw.encode("utf-8"))
    logo = next(part for part in message.walk() if part.get("Content-ID") == "<demo-company-logo>")
...
def test_subject_normalization() -> None:
    ...
    assert normalized_subject("回复：Re: DEMO-P300，600 kg") == "demo-p300，600 kg"
    assert has_thread_subject_prefix("回复：Re: DEMO-P300，600 kg") is True
    assert has_thread_subject_prefix("DEMO-P300，600 kg") is False
