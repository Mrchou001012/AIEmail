...
async def _seed_catalog_and_interest(
    ...
    *,
    ...
    contact = Contact(
        ...
        name="Alice Buyer",
        email="buyer@catalog-customer.example",
        language="en",
    )
...
async def _seed_departed_reactivation_parent(
    ...
    *,
    interests: list[str],
    old_email: str = "original@buyer-company.example",
    reply_email: str = "reply@buyer-company.example",
) -> tuple[int, int, Outbox]:
    await import_product_catalog(db_session, apply=True)
    customer = Customer(
        company_name="Example Buyer Co., Ltd.",
        language="en",
    ...
    parent = Outbox(
        ...
        message_id="<wake-departed@example.org>",
        recipient=old_email,
...
def _departed_reply_message(parent_message_id: str, body: str) -> bytes:
    ...
    message["From"] = "Demo Buyer <reply@buyer-company.example>"
    message["To"] = "sales-agent@example.com"
    message["Subject"] = "Re: Checking in from Example Chemicals"
...
def _message(
    ...
    *,
    ...
    message = EmailMessage()
    message["From"] = "Alice Buyer <buyer@catalog-customer.example>"
    message["To"] = "sales-agent@example.com"
...
async def test_productless_quote_with_category_interest_sends_one_clarification(
    ...
    assert "confirm the product name or Example product code" in clarification.raw_message
...
async def test_productless_quote_recovers_product_from_complete_quoted_thread(
    ...
    email_row = await ingest_raw_email(
        ...
        _message(
            ...
            (
                ...
                "> We are interested in DEMO-P100."
            ),
    ...
    assert email_row is not None and email_row.case_id is not None
    assert "DEMO-P100" not in email_row.body_text
    await process_inbound(db_session, email_row.id)

    case = await db_session.get(SalesCase, email_row.case_id)
    assert case is not None
    await db_session.refresh(case, ["product"])
    assert case.product is not None and case.product.code == "DEMO-P100"
...
async def test_manual_draft_regeneration_preserves_catalog_and_defaults_new_customer_to_prepayment(
    ...
    email_row = await ingest_raw_email(
        db_session,
        _message(
            "Checking in from Example Chemicals",
...
async def test_product_list_payment_term_uses_latest_sent_customer_quote(
    ...
    product = await db_session.scalar(
        select(Product).where(Product.code == "DEMO-P100")
    )
    ...
    db_session.add(
        Outbox(
            ...
            recipient="buyer@catalog-customer.example",
            raw_message="From: sales-agent@example.com\nTo: buyer@catalog-customer.example\n\nQuote",
            status=DeliveryStatus.SENT,
...
async def test_selected_legacy_handoff_can_use_company_research_backfill(
    ...
    async def research(*args, **kwargs):
        ...
        return (
            ...
            [
                CompanyResearchSource(url="https://industry.example/example-buyer"),
                CompanyResearchSource(url="https://trade.example/example-buyer"),
            ],
            {
                "provider": "anthropic",
                "model": "claude-test",
...
async def test_product_list_backfill_previews_then_queues_old_open_handoff(
    ...
    outbox = await _queued_product_list(db_session)
    assert outbox is not None
    assert "DEMO-P100" in outbox.raw_message
    await db_session.refresh(handoff)
...
async def test_product_list_backfill_maps_specific_product_in_selected_category(
    ...
    email_row = await ingest_raw_email(
        ...
        _message(
            "Product catalog",
            "Please send your product list for DEMO-P400.",
    ...
    assert preview["candidate_count"] == 1
    assert preview["candidates"][0]["detected_product_code"] == "DEMO-P400"
    ...
    assert case.product.code == "DEMO-P400"
...
async def test_product_specific_list_request_sends_official_catalog(
    ...
    email_row = await ingest_raw_email(
        db_session,
        _message(
            "DEMO-P100 product list",
            "PRODUCT DEMO-P100. Please send your product list.",
    ...
    assert product is not None and product.code == "DEMO-P100"
...
async def test_catalog_import_is_idempotent(db_session: AsyncSession) -> None:
    ...
    mtms = await db_session.scalar(select(Product).where(Product.code == "DEMO-MTMS"))
    assert mtms is not None
    mtms.cas_no = "111-11-1"
    await db_session.flush()
...
async def test_full_customer_workbook_stores_interest_category(
    ...
    sheet.append(
        [
            "Example Buyer",
            "Alice Buyer",
            "buyer@catalog-customer.example",
            "",
            "工业硅烷",
            date(2020, 1, 1),
            date(2024, 1, 1),
    ...
    customer = await db_session.scalar(
        select(Customer).where(Customer.company_name == "Example Buyer")
    )
    assert customer is not None
    ...
    assert brisben_interests[0]["value"] == "General Reagent 400"
    brisben_contact = await db_session.scalar(
        select(Contact).where(Contact.email == "buyer-two@example.com")
    )
    acac = await db_session.scalar(select(Product).where(Product.code == "DEMO-P400"))
    ...
    contact = await db_session.scalar(
        select(Contact).where(Contact.email == "buyer@catalog-customer.example")
    )
    assert contact is not None
    email_row = await ingest_raw_email(
        db_session,
...
async def test_departed_reply_from_same_domain_retires_old_contact_and_auto_sends(
    ...
    new_contact = await db_session.scalar(
        select(Contact).where(
            func.lower(Contact.email) == "reply@buyer-company.example"
        )
    )
    ...
    outbox = await _queued_product_list(db_session)
    assert outbox is not None
    assert "reply@buyer-company.example" in outbox.raw_message
    assert "Catalog.pdf" in outbox.raw_message
