...
async def test_human_approved_reply_is_audited_and_sends_with_auto_send_disabled(
    ...
    assert "Demo Sales Team" in outbox.raw_message
    parsed = parse_mime(outbox.raw_message.encode("utf-8"))
...
async def test_new_subject_with_multiple_products_requires_human_selection(
    ...
    message.set_content("Please quote DEMO-P300 and DEMO-P301, quantity 100 kg each.")
    ...
    assert handoff.extracted_facts["product_codes"] == ["DEMO-P300", "DEMO-P301"]
...
async def test_reply_to_case_less_reactivation_can_create_product_case(
    ...
    second["Subject"] = "Re: Checking in from Example Chemicals"
...
async def test_case_less_reactivation_reply_from_same_company_domain_creates_endpoint(
    ...
    ids, parent, recipient, original_contact = await _seed_changed_sender_reactivation(
        db_session,
        original_email="original@chemical-buyer.example",
        token="same-company-domain",
    )
    message = MIMEMessage()
    message["From"] = "reply@chemical-buyer.example"
    message["To"] = "sales-agent@example.com"
    message["Subject"] = "Re: Checking in from Example Chemicals"
    ...
    assert reply_contact.email == "reply@chemical-buyer.example"
    assert reply_contact.customer_id == ids["customer_id"]
    assert reply_contact.id != original_contact.id
    assert original_contact.email == "original@chemical-buyer.example"
    ...
    assert links[-1]["matched_domain"] == "chemical-buyer.example"
    await db_session.refresh(parent)
    await db_session.refresh(recipient)
    assert parent.case_id == sales_case.id
    assert recipient.case_id == sales_case.id
...
@pytest.mark.parametrize(
    ...
    [
        ("buyer@company-one.example", "sales@company-two.example", "cross-domain"),
...
async def test_history_uses_explicit_product_to_choose_one_of_multiple_cases(
    ...
    second_case = SalesCase(
        ...
        stage=CaseStage.QUOTING,
        status=CaseStatus.ACTIVE,
        subject_key="demo-p300 quotation",
    )
    ...
    row = EmailMessage(
        ...
        from_address="internal@example.com",
        to_addresses=["sales-agent@example.com"],
        subject="Please quote DEMO-P300",
        body_text="We need DEMO-P300 pricing.",
        attachment_metadata=[],
...
async def test_soft_bounce_for_suppressed_recipient_skips_review_notification(
    ...
    assert address.last_bounce_diagnostic == (
        "smtp; 452 4.2.2 The recipient's inbox is out of storage space"
    )
